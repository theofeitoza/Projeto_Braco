...
def main(page):
    ...
    button_value = '90'
    moviment_value = 0
    saved_properties = (slider_values, button_value)
    sliders = []
    ...
    def handle_button_change(e):
        nonlocal button_value
        try:
            button_value = int(e.data.strip('[]\"'))  # Remove colchetes e aspas
        except ValueError as ve:
            print(f"Erro ao converter button_value: {ve}")
            return  # Se não puder converter, sai da função
            
        # Verifica se `button_value` é 90 (Fechado) ou 180 (Aberto)
        if button_value in [90, 180]:
            send_servo_value(5, button_value)  # Envia o valor 90 ou 180 para o servo 5
            print(f"Servo 5 movido para {'Fechado' if button_value == 90 else 'Aberto'} ({button_value})")
        else:
            print(f"Valor inválido para o servo 5: {button_value}. Apenas 90 (Fechado) ou 180 (Aberto) são permitidos.")
    ...
    def save_properties(e):
        ...
        saved_properties = (slider_values.copy(), button_value)
        print(f"Propriedades salvas: {saved_properties}")
        page.update()

    def reset_positions(e):
        nonlocal slider_values, button_value
        slider_values = [0, 0, 0, 0]
        button_value = "Aberta"
    ...
    def write_positions(e):
        try:
            ...
            linhas[linha_escolhida] = f"{slider_values},{button_value}"
            with open('posicoes.txt', 'w') as arquivo:
                for linha in linhas:
                    arquivo.write(f"{linha}\n")
            
            show_bottom_sheet(f'Valores exportados: {slider_values}, {button_value} (linha {linha_escolhida + 1})')
    ...
    def play_movements(e):
        ...
        try:
            ...
            for linha in linhas:
                ...
                if not linha:
                    continue
                partes = linha.split('],')  # Ajuste para lidar com o formato correto
                if len(partes) != 2:
                    print(f"Erro: Formato de linha inválido.")
                    continue
                sliders_str = partes[0].strip('[]')
                button_str = partes[1].strip()  # Remove espaços em branco do valor do botão
                
                try:
                    slider_values = [float(value.strip()) for value in sliders_str.split(',')]
                    button_value = int(button_str)  # Converter o valor do botão para inteiro
                except ValueError as ve:
                    print(f"Erro: Não foi possível converter os valores dos sliders ou do botão.")
                ...
                # Envia o valor para o servo 5
                send_servo_value(5, button_value)
    ...
    def import_positions(e):
        nonlocal slider_values, button_value, moviment_value
        try:
            linha_escolhida = int(moviment_value)
            if linha_escolhida < 0 or linha_escolhida > 2:
            ...
            partes = linha.split('],')  # Ajuste aqui para lidar com o seu formato
            if len(partes) != 2:
                print(f"Erro: Formato de linha inválido na linha {linha_escolhida}. Linha: '{linha}'")
                return
    ...
    slider2 = ft.Slider(min=0, max=180, width=300, value=90, data=3, on_change=handle_change, label="{value}º", adaptive=True)
    ...
    def create_segmented_button():
        ...
        segmented_button = ft.SegmentedButton(
            on_change=handle_button_change,
            selected={button_value},
            width=300,
            allow_multiple_selection=False,
            segments=[
                ft.Segment(value=90, label=ft.Text("Fechada"), icon=ft.Icon(ft.icons.LOOKS_ONE)),
                ft.Segment(value=180, label=ft.Text("Aberta"), icon=ft.Icon(ft.icons.LOOKS_TWO)),
            ]
        )
        ...
        ft.Container(
            ft.Row([ft.Column([ft.ElevatedButton(text="SAVE POSITIONS", on_click=save_properties, width=300, height=80, style=ft.ButtonStyle(bgcolor=ft.colors.BLACK, color=ft.colors.WHITE, shape=ft.ContinuousRectangleBorder(radius=0))),
                ...
                ft.Column([ft.Row([slider0]),
                            ...
                            ft.Row([slider2]),
                            ft.Row([slider3]),
                            ft.Row([create_segmented_button()]),
